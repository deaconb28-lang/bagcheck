...
Two faces, one product. Inside, Bagcheck is a black instrument that scores behaviour. Outside, it sells the artefact those numbers mint: the public angle is Wrapped-first — **"Turn your portfolio into a flex"** — connect a brokerage in two taps, get a year worth posting. The score is introduced on the landing as the coming-soon second act ("Whoop for your portfolio"), behind a waitlist. The brand is the lowercase wordmark **bagcheck** and the bag mark — white bag, amber clasp on a near-black square — drawn in `app/icon.svg` and as `BagMark` in the landing; the wordmark is never capitalised in UI chrome (prose may still say Bagcheck).
...
- **The marketing field is the second world: a white editorial page in the Cal AI register, spoken entirely in `--mk-*` tokens.** They live in `styles/tokens.css` and only `app/(marketing)` reads them; the app's tokens never appear on the landing and `--mk-*` never enters the app. Black ink on white; `--mk-green` for money-up (its `-deep` variant on small text, for contrast); `--mk-red` for money-down; violet only inside the Wrapped artefacts and phone mocks — a violet card on the white page is the product shot, not a palette leak. The phone mocks are `aria-hidden` illustrations of the app, built in JSX, never screenshots.
