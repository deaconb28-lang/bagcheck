...
- Card art is generated **per person, from their own numbers**, for all twelve kinds — not for Wrapped alone, and never from a static per-kind file. `lib/wrapped/brief.ts` turns four measured quantities (magnitude, direction, texture, density) into the composition, so a 412-day hold and a 34-day hold draw visibly different mountains. The model shapes the picture and **never draws a glyph**: a generated figure is one nobody can correct, on an artefact whose whole claim is that its numbers came from a brokerage. Art is cached by `briefKey`, not by user, so two identical-looking cards cost one image.
