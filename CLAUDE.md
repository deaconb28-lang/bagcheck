...
- `app/og/[slug]/render.tsx`, `lib/logos/fetch.ts`, `lib/avatars/drawn.ts` and `lib/avatars/prompt.ts` are the files allowed to repeat palette hexes, and the reason is the same in all four: an image at a URL carries no stylesheet, and a prompt describing a colour to a model cannot cite a custom property. Keep the values in step with the `--share-*` tokens, and give every multi-child element an explicit `display`.
