...
installed, so that a fresh clone of Canopy has the design skills available
...
`ui-ux-pro-max` is the one Canopy actually leans on. Its searchable
...
update and would make the version above a lie. Anything Canopy-specific
...
ui-ux-pro-max plugin, which is true of upstream and false of Canopy.
